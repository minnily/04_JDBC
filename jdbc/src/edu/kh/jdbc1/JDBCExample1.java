...
public class JDBCExample1 {
	public static void main(String[] args) {
		...
		// DB 연결 정보를 담을 객체
		// DB연결 정보? DBMS 타입, 이름, IP, Port, 계정명, 비밀번호 저장
		// => DBeaver의 계정 접속 방법과 유사함
		// * Java와 DB 사이를 연결해주는 통로 역할을 할 것임!*
		
		
		Statement stmt = null;
		//Connection을 통해 SQL문을 DB에 전달하여 
		//실행하고 생성된 결과 (ResultSet, 성공한 행의 개수)를 반환(Java)하는데 사용되는 객체
		...
		try{
			...
			// --> 생략해도 자동으로 메모리 로드가 진행됨(하지만, 명시적으로 작성하는 걸 권장하기에 적어주면 좋음!)
			...
			// 중간확인
			//System.out.println(conn); //oracle.jdbc.driver.T4CConnection@24a35978
			...
			while(rs.next()) {
				//rs.next() : rs가 참조하고 있는 ResultSet 객체의 첫 번째 컬럼부터 순서대로 한행씩 이동하며
